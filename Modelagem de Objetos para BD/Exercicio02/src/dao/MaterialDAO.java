...
public class MaterialDAO extends BaseDAO {
    ...
    public List<Material> materialsPerProject(int id_project) throws SQLException {
        List<Material> list = new ArrayList<>();
        String sql = """
                select m.* from material e join material_consumption mc on m.id_material = mc.id_material where mc.id_project = ?
                """;
        try(Connection con = con();
            PreparedStatement pre = con.prepareStatement(sql);
            ResultSet rs = pre.executeQuery()) {
